...
function emptyState() {
  return {
    settings: {
      ...
      depotLng: null,
      fuelCostPerKm: 0.15,
      dispatchDate: null, // "YYYY-MM-DD" — the day every delivery's HH:MM window is resolved against
    },
    vehicles: [],
    drivers: [],
    deliveries: [],
...
export function createDelivery(data) {
  const delivery = {
    ...
    windowStart: data.windowStart || null, // "HH:MM", today, optional earliest arrival
    windowEnd: data.windowEnd || null, // "HH:MM", today, optional latest arrival
...
export function clearRoute(vehicleId) {
  delete store.routes[vehicleId];
}
