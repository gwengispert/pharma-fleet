...
// Reverse geocode: lat/lng -> nearest formatted address. Used by the "pick on
// map" address picker, which sends the clicked/dragged coordinates here.
export async function reverseGeocode(lat, lng) {
  ...
  if (data.status !== "OK" || !data.results?.length) {
    throw new Error(
      `Reverse geocoding failed for ${lat},${lng}: ${data.status}${data.error_message ? " - " + data.error_message : ""}`
    );
  }

  const { lat: resolvedLat, lng: resolvedLng } = data.results[0].geometry.location;
  const formattedAddress = data.results[0].formatted_address;
  return { lat: resolvedLat, lng: resolvedLng, formattedAddress };
}
